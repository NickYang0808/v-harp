...
let prevLandmarksList = []; // 放在 onResultsHands() 外面（全域變數）

function smoothLandmarks(currLandmarks, prevLandmarks, alpha = 0.4) {
  if (!prevLandmarks) return currLandmarks.map(p => ({...p}));

  return currLandmarks.map((curr, i) => ({
...
function onResultsHands(results) {
  ...
  if (results.multiHandLandmarks && results.multiHandedness) {
    for (let index = 0; index < results.multiHandLandmarks.length; index++) {
      ...
      // ⭐ 平滑 landmark
      landmarks = smoothLandmarks(landmarks, prevLandmarksList[index]);
      prevLandmarksList[index] = landmarks;
